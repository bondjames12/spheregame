...
namespace AnimationProcessor
{
  ...
  public class AnimationProcessor : LevelProcessor.Dxt5ModelProcessor
  {
    ...
    {
      if (String.IsNullOrEmpty(excludeAnimations_))
        excludeAnimationsExpressions_ = null;
      else
      {
        string[] exprs = excludeAnimations_.Split(new char[] { ';' });
        excludeAnimationsExpressions_ = new Regex[exprs.Length];
        for (int i = 0; i != exprs.Length; ++i)
          excludeAnimationsExpressions_[i] = new Regex(exprs[i], RegexOptions.IgnoreCase);
      }
      if (String.IsNullOrEmpty(excludeBones_))
        excludeBonesExpressions_ = null;
      else
      {
        string[] exprs = excludeAnimations_.Split(new char[] { ';' });
        excludeBonesExpressions_ = new Regex[exprs.Length];
        for (int i = 0; i != exprs.Length; ++i)
          excludeBonesExpressions_[i] = new Regex(exprs[i], RegexOptions.IgnoreCase);
      }
    }
