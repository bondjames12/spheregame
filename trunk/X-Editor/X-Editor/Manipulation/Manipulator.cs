...
namespace X_Editor
{
    ...
    public sealed partial class Manipulator
    {
        ...
        /// </summary>
        private class Input : XIInputProvider
        {
            ...
            {
                get { return mLeftClick; }
            }

            public bool LeftRelease
            {
                get { return mLeftRelease; }
            }

            public int X { get { return (int)mEnd.X; } }
            public int Y { get { return (int)mEnd.Y; } }
            ...
            public Viewport Viewport
            {
                get
                {
                    if ((mGraphics.GraphicsDevice == null)
                        || (mGraphics.GraphicsDevice.IsDisposed))
                    {
                        return new Viewport();
                    }

                    return mGraphics.GraphicsDevice.Viewport;
                }
            }

            public Input(IGraphicsDeviceService graphics)
            {
                mGraphics = graphics;
            ...
            public void Reset()
            {
                ...
                mDelta = Vector2.Zero;
                mLeftButton = mLeftClick = (mouse.LeftButton == ButtonState.Pressed);
                mLeftRelease = !mLeftButton;
            }
            ...
            public void Cycle()
            {
                ...
                MouseState mouse = Mouse.GetState();
                mEnd = new Vector2(mouse.X, mouse.Y);

                mDelta = mEnd - mStart;

                mLeftClick = (mLeftButton) ? (false) : (mouse.LeftButton == ButtonState.Pressed);
                mLeftRelease = (mLeftButton) ? (mouse.LeftButton == ButtonState.Released) : (false);
                mLeftButton = mouse.LeftButton == ButtonState.Pressed;
            }
        }
        ...
        private XIInputProvider                  mInput;

        private XITransform                      mTransform;
        private XICamera                         mCamera;
        ...
        /// <summary>
        /// Gets or sets the IInputProvider used by the Manipulator to process input
        /// </summary>
        public XIInputProvider InputProvider
        {
            get { return mInput; }
            set { mInput = value; }
        }
        ...
        public void Draw()
        {
            ...
            if (mSettings.DrawBoundsEnabled && (mTransform is XIBoundedTransform))
                DrawBounds((XIBoundedTransform)mTransform);

            // set the internal depth buffer so the manipulator will draw on top of everything
            // without disrupting the depth buffer of the frame buffer
        ...
        private void DrawBounds(XIBoundedTransform tform)
        {
