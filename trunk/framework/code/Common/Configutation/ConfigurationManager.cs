...
namespace QuickStart
{
    ...
    public sealed class ConfigurationManager
    {
        ...
        private const string SectionManagers = "managers";
        ...
        public List<BaseManager> GetManagers(QSGame game)
        {
            if (game == null)
            {
            ...
            List<string> path = new List<string>(new string[] { ConfigurationManager.SectionConfiguration, ConfigurationManager.SectionManagers });
            XmlNode managersNode = this.GetNode(path);
            if (managersNode == null)
            {
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "section '{0}' was not found", ConfigurationManager.SectionManagers));
            }

            List<BaseManager> managers = new List<BaseManager>();

            for (int i = managersNode.ChildNodes.Count - 1; i >= 0; i--)
            {
